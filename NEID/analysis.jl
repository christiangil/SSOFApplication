...
rvs, rv_errors, tel_errors, star_errors = SSOFA.estimate_σ_curvature(mws; save_fn=base_path * "results_curv.jld2", save_model_fn=save_path, recalc=recalc, multithread=false)
mws.om.metadata[:todo][:err_estimated] = true
rvs_b, rv_errors_b, tel_s_b, tel_errors_b, star_s_b, star_errors_b = SSOFA.estimate_σ_bootstrap(mws; save_fn=base_path * "results_boot.jld2", save_model_fn=save_path, recalc_mean=true, recalc=recalc, return_holders=true)
